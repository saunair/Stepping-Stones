...
#define V_I_GAIN1 0.001
#define V_D_GAIN1 0.0

#define V_P_GAIN2 0
#define V_I_GAIN2 0.001
#define V_D_GAIN2 0
...
//The thresholds for error accumulations
#define Position_threshold 70
#define Velocity_threshold 21000
...
void check_reset_system() 
{
  ...
  //check the error accumulation 
 /*  if (positionErrorSum[1] > Position_threshold ||
       positionErrorSum[2] > Position_threshold ||
       velocityErrorSum[1] > Velocity_threshold ||
       velocityErrorSum[2] > Velocity_threshold) {

          //set the flag of failure
        skate_fault = 1;          
   */     
  //     }

   if (positionError[1] > Position_threshold)
      skate_fault |= 1<<0; 
   if (positionError[2] > Position_threshold)
      skate_fault |= 1<<1; 
   if (velocityErrorSum[1] > Velocity_threshold)
      skate_fault |= 1<<2; 
   if (velocityErrorSum[2] > Velocity_threshold)
      skate_fault |= 1<<3; 
